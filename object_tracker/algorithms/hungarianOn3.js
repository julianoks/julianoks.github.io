 ...
 */
export function hungarianOn3(costMatrix, isProfit) {
...
function AP(costMatrix, isProfit) {
  ...
  this.committedWorkers = new BitSet('0'.repeat(dim));
  this.jobByWorkerBS = new BitSet('0'.repeat(dim));
  this.workerByJobBS = new BitSet('0'.repeat(dim));
...
AP.prototype.execute = function () {
  ...
  this.greedyMatch();
  var w = Array(this.dim).findIndex((_,i) => this.jobByWorkerBS.get(i) == 0);
  if(w >= this.dim){ w = -1; }
  while (w !== -1) {
    this.initializePhase(w);
    this.executePhase();
    w = Array(this.dim).findIndex((_,i) => this.jobByWorkerBS.get(i) == 0);
    if(w >= this.dim){ w = -1; }
  }
